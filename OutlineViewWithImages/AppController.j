...
- (void)applicationDidFinishLaunching:(CPNotification)aNotification
{
    ...
    var dataView = [[CKDataView alloc] initWithFrame:CGRectMakeZero()];
...
@implementation CKDataView : CPView
{
    CPTextField     label;
...
- (void)setObjectValue:(JSObject)object
{
    ...
    if (!label)
    {
        label = [[CPTextField alloc] initWithFrame:CGRectInset([self bounds], 16.0, 1.0)];
    ...
    if (!icon)
    {
        ...
        iconView = [[CPImageView alloc] initWithFrame:CGRectMake(0,0,16.0,16.0)];
        [iconView setImage:icon];
        [self addSubview:iconView];
    }
    
    [label setStringValue:object];
    [label sizeToFit];
}
